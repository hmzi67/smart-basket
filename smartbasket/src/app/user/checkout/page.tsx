...
function Checkout() {
    ...
    return (
      <main className="min-h-screen bg-[#f6f8f7] pb-16">
        ...
        <div className="mx-auto max-w-6xl px-4 pt-8 sm:px-6">
          <div className="mb-7 flex items-center gap-3 text-xs font-semibold text-slate-400"><Link href="/user/cart" className="text-emerald-700">01 Basket</Link><span className="h-px w-8 bg-slate-200" /><span className="text-emerald-700">02 Checkout</span><span className="h-px w-8 bg-slate-200" /><span>03 Confirmation</span></div>
          <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
              <div className="mb-6 flex items-center gap-3"><span className="rounded-xl bg-emerald-50 p-2.5 text-emerald-700"><MapPin size={21} /></span><div><h2 className="font-semibold text-slate-900">Where should we deliver?</h2><p className="mt-1 text-xs text-slate-500">Enter your address and confirm the pin on the map.</p></div></div>
              <div className="grid gap-4 sm:grid-cols-2">
                {([{ key: 'fullName', label: 'Full name', placeholder: 'Recipient name', autoComplete: 'name' }, { key: 'mobile', label: 'Mobile number', placeholder: '03XXXXXXXXX', autoComplete: 'tel' }, { key: 'city', label: 'City', placeholder: 'City', autoComplete: 'address-level2' }, { key: 'state', label: 'Province / region', placeholder: 'Province or region', autoComplete: 'address-level1' }, { key: 'pincode', label: 'Postal code', placeholder: 'Postal code', autoComplete: 'postal-code' }] as const).map(field => <label key={field.key} className="block text-xs font-semibold text-slate-600">{field.label}<input type={field.key === 'mobile' ? 'tel' : 'text'} autoComplete={field.autoComplete} value={address[field.key]} onChange={event => setAddress(prev => ({ ...prev, [field.key]: event.target.value }))} placeholder={field.placeholder} className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-3 text-sm font-normal text-slate-800 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" /></label>)}
                <label className="block text-xs font-semibold text-slate-600 sm:col-span-2">Street address<textarea autoComplete="street-address" rows={2} value={address.fullAddress} onChange={event => setAddress(prev => ({ ...prev, fullAddress: event.target.value }))} placeholder="House, street and nearby landmark" className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-3 text-sm font-normal text-slate-800 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100" /></label>
              </div>
              <div className="mt-6 border-t border-slate-100 pt-5"><label htmlFor="location-search" className="text-xs font-semibold text-slate-600">Find your location</label><form onSubmit={event => { event.preventDefault(); void handleSearchQuery() }} className="mt-2 flex gap-2"><input id="location-search" value={searchQuery} onChange={event => setSearchQuery(event.target.value)} placeholder="Search city or area" className="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-3 text-sm outline-none focus:border-emerald-500" /><button type="submit" disabled={searchLoading === 'true'} className="rounded-xl bg-emerald-700 px-4 text-sm font-semibold text-white disabled:opacity-50">{searchLoading === 'true' ? <Loader2 size={18} className="animate-spin" /> : 'Search'}</button></form><div className="relative isolate mt-4 h-72 overflow-hidden rounded-xl border border-slate-200 bg-slate-50">{position ? <CheckOutMap position={position} setPosition={setPosition} /> : <p role="status" className="flex h-full items-center justify-center text-sm text-slate-500">Finding your location…</p>}<button type="button" aria-label="Use my current location" onClick={handleCurrentLocation} className="absolute bottom-6 right-3 z-10 rounded-xl bg-white p-3 text-emerald-700 shadow-md hover:bg-emerald-50"><LocateFixed size={20} /></button></div>{locationError && <p role="alert" className="mt-2 text-xs text-amber-700">{locationError}</p>}<p className="mt-2 text-xs text-slate-500">Drag the pin to your delivery location.</p></div>
            </section>
            <aside className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:sticky lg:top-6">
              <h2 className="text-lg font-semibold text-slate-900">Payment & summary</h2><p className="mt-1 text-xs text-slate-500">Choose how you’d like to pay.</p>
              <div className="mt-5 space-y-3">{([{ value: 'cod', label: 'Cash on delivery', detail: 'Pay when your groceries arrive', icon: Truck }, { value: 'online', label: 'Easypaisa', detail: 'Pay with your mobile wallet', icon: CreditCard }] as const).map(({ value, label, detail, icon: Icon }) => <button key={value} type="button" aria-pressed={paymentMethod === value} onClick={() => { setPaymentMethod(value); setOrderError(''); setEpMessage('') }} className={`flex w-full items-center gap-3 rounded-xl border p-4 text-left transition ${paymentMethod === value ? 'border-emerald-600 bg-emerald-50' : 'border-slate-200 hover:bg-slate-50'}`}><Icon size={20} className="shrink-0 text-emerald-700" /><span className="flex-1"><span className="block text-sm font-semibold text-slate-800">{label}</span><span className="mt-1 block text-xs text-slate-500">{detail}</span></span><span className={`h-4 w-4 rounded-full border ${paymentMethod === value ? 'border-[5px] border-emerald-600' : 'border-slate-300'}`} /></button>)}</div>
              ...
              <button type="button" disabled={epLoading || orderLoading || !cartData.length} onClick={() => { if (paymentMethod === 'cod') void handleCod(); else void handleOnlineOrder() }} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-700 py-3.5 text-sm font-semibold text-white hover:bg-emerald-800 disabled:cursor-not-allowed disabled:opacity-50">{epLoading || orderLoading ? <><Loader2 size={18} className="animate-spin" /> Processing…</> : paymentMethod === 'cod' ? 'Place order' : 'Pay & place order'}</button>
