...
export async function POST(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!body || !Object.hasOwn(body, 'isActive')) throw new UserInputError('isActive is required')
    const user = await updateManagedUser(id, { isActive: body.isActive }, session.user.id)
