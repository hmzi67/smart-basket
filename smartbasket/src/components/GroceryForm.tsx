...
const field = 'mt-1.5 w-full rounded-lg border border-gray-300 bg-gray-50 px-3 py-2.5 text-sm text-gray-800 outline-none focus:ring-2 focus:ring-green-500'
export default function GroceryForm({ initial, onSaved, onBusyChange }: { initial?: IGrocery; onSaved: (grocery: IGrocery) => void; onBusyChange?: (busy: boolean) => void }) {
  ...
  return <form onSubmit={submit} className="space-y-5">
    <fieldset disabled={busy} className="space-y-4">
      {initial && image && <div className="relative h-64 overflow-hidden rounded-lg border border-gray-100 bg-white"><Image src={image} alt="Product preview" fill sizes="400px" className="object-contain p-3" /></div>}
      ...
      <label className="block text-sm font-medium">Product image<input name="image" type="file" accept="image/*" required={!initial} onChange={event => { const file = event.target.files?.[0]; setImage(file ? URL.createObjectURL(file) : initial?.image ?? '') }} className="mt-2 block w-full text-sm text-gray-500 file:mr-3 file:rounded-lg file:border-0 file:bg-green-50 file:px-3 file:py-2 file:text-green-700" /><span className="mt-1 block text-xs font-normal text-gray-500">Up to 5 MB.{initial && ' Leave empty to keep the current image.'}</span></label>
      {!initial && image && <div className="relative h-36 overflow-hidden rounded-xl bg-gray-50"><Image src={image} alt="Product preview" fill sizes="400px" className="object-contain" /></div>}
      <label className="flex items-center gap-2 text-sm"><input name="isAvailable" type="checkbox" defaultChecked={initial?.isAvailable ?? true} className="h-4 w-4 accent-green-700" /> Available for sale</label>
      <p className="text-xs text-gray-500">Out-of-stock and unavailable products are hidden from the customer catalog.</p>
    </fieldset>
    {error && <p role="alert" className="rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}
    <button type="submit" disabled={busy} className="w-full rounded-lg bg-linear-to-r from-green-500 to-green-700 py-3 text-sm font-semibold text-white hover:bg-green-700 disabled:opacity-50">{busy ? 'Saving...' : initial ? 'Save changes' : 'Add grocery'}</button>
  </form>
}
